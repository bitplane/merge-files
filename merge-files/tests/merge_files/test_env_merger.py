from merge_files.env_merger import env_merger


def test_env_merger_no_change():
    value = "\n".join(["KEY=yay", "HOST=localhost"]).encode()

    result = env_merger(value, value)

    assert result == value


def test_env_merger_overwrite_value():
    dest = "\n".join(["KEY=custom"]).encode()
    source = "\n".join(["KEY=default"]).encode()

    expected = "\n".join(["KEY=default"]).encode()

    actual = env_merger(source, dest, True)

    assert actual == expected


def test_env_merger_change_variable():
    dest = "\n".join(["KEY=mine", "HOST=localhost"]).encode()
    source = "\n".join(["KEY=default", "NEW=cool"]).encode()

    expected = "\n".join(["KEY=mine", "HOST=localhost", "NEW=cool"]).encode()

    actual = env_merger(source, dest)

    assert actual == expected


def test_env_merger_add_comments():
    dest = "\n".join(["KEY=mine", "HOST=localhost"]).encode()
...
def test_env_merger_keep_comments():
...
def test_env_merger_keep_comments_with_equals():
...
def test_env_merger_dont_duplicate_comments():
    dest = "\n".join(["#UNCOMMENT_ME=n # explode()", "OTHER_VAR"]).encode()
